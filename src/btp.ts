...
// 外部アプリ SmartDB REST API v3（Apps & Services）。静的 bearer（バインダトークン）は
// Destination 側で解決するため、呼出側は path/method/body のみ指定する。
export async function callSmartdbApi(connection: ConnectionConfig, args: BtpCallArgs) {
  return _call(connection, '/call-smartdb-api', args);
}
